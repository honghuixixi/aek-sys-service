...
public class SysUserModuleServiceImpl extends BaseServiceImpl<SysUserModuleMapper, SysUserModule>
	...
	@Override
	public void checkHasModule(Long moduleId, Long userId,Long tenantId) {
		Boolean orgAdmin = sysRoleMapper.isOrgAdmin(userId, tenantId);
		SysUser user = sysUserMapper.selectById(userId);
		SysTenant tenant = sysTenantMapper.selectById(tenantId);
		Long adminId = tenant.getAdminId();
		//SysUserModule checkHasModule = sysUserModuleMapper.checkHasModule(moduleId, userId);
		if (user.getAdminFlag() || orgAdmin || user.getId().equals(adminId)) {// 管理员显示机构下所有的模块ids
			List<Long> orgModuleIds = sysRoleMapper.getOrgModuleIds(userId, tenantId);
			if(orgModuleIds != null && orgModuleIds.size() > 0){
				Boolean flag=false;
				for (Long id : orgModuleIds) {
					if(id.longValue() == moduleId.longValue()){
						flag=true;
					}
				}
				if(!flag){
					throw new BusinessException("201","此业务已闭关，请联系机构管理员");
				}
			}
		}else{//非管理员
			List<Long> orgModuleIdsByRole = sysRoleMapper.getOrgModuleByRole(userId, tenantId);
			Boolean flag=false;
			for (Long id : orgModuleIdsByRole) {
				if(id.longValue() == moduleId.longValue()){
					flag=true;
				}
			}
