...
public class SysUserServiceImpl extends BaseServiceImpl<SysUserMapper, SysUser> implements SysUserService {
	...
	@Override
	public void checkUserPermission(Long userId, Long tenandId, String permissionCode) {
		if(!getUserPermission(userId, tenandId, permissionCode))throw new BusinessException("201","无操作权限，请联系机构管理员");
		
	}

	@Override
	public Boolean getUserPermissions(List<Long> list, String rht,Long tenantId) {
		int count = this.userMapper.getUserPermissionCounts(list,  rht, tenantId);
		int size=list.size();
		if(count >= size){
			return true;
		}
